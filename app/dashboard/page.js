...
export default function TeacherDashboard() {
  ...
  if (loading) return <div className="min-h-screen bg-slate-900 flex items-center justify-center text-white font-black">SYNCING GEMINI LAB...</div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-3">
          <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100">
            ...
            {roster.map(s => (
              <div key={s.id} className="p-3 border rounded-xl flex justify-between items-center mb-2">
        ...
        <div className="lg:col-span-4">
          <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100">
            <h2 className="font-black text-[10px] uppercase tracking-widest text-slate-400 mb-6 flex items-center gap-2"><LayoutGrid size={14}/> Lab Tasks</h2>
            <textarea value={newTask} onChange={e => setNewTask(e.target.value)} className="w-full p-3 border rounded-xl text-xs mb-3 h-20" placeholder="Describe your weekend in Houston..." />
            <button onClick={addTask} className="w-full bg-blue-600 text-white p-3 rounded-xl font-black text-[10px] uppercase tracking-widest mb-6">Save Task</button>
            {prompts.map(p => (
        ...
        <div className="lg:col-span-5">
          <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100 min-h-[500px]">
            <h2 className="font-black text-[10px] uppercase tracking-widest text-slate-400 mb-6">Submissions</h2>
            {submissions.map(sub => (
              <div key={sub.id} className="p-6 border rounded-3xl bg-slate-50 mb-4">
                <p className="text-xs font-black uppercase mb-1">{sub.students?.full_name}</p>
                <p className="text-[10px] text-slate-400 italic mb-4">"{sub.assignments?.prompt_text}"</p>
